...
stopuser = {}
token = '7259579965:AAGDSnBSbXhrx1qrkxn8Rew3xS_fWATpzNc'
...
mainc = "https://t.me/otpsjksmhd_bot"  # تأكد من أن الرابط صحيح
...
def menu_callback_hour(call):
    ...
    bot.send_invoice(
        chat_id=Sid,
        title=title,
        description="~ Bot ABDOU VIP Subscribe",
...
♤ Youre Not Subscribed in Check Abdou Bot ❌
♤ Channel ~ @VZ_PA
♤ For Show Bot Prices Send -> /buy
♤ Programmers ~ @abdouch_18 </b>''', reply_markup=keyboard)
...
You are Not Subscribed Abdou CHK !❌

Your ID : {message.chat.id}
Programmer - @abdouch_18''')
...
def generate_code(message):
    def my_function():
        ...
        try:
            ...
            pas = 'Abdou-' + '2024' +'-' + ''.join(random.choices(characters, k=4))
            current_time = datetime.now()
    ...
    def my_function():
        try:
            ...
            if re in json_data:
                ...
                msg = f'''<b>♤ Check Abdou CHK Subscribtion Done ✅
♤ Expires in -> {timer}</b>'''
                bot.reply_to(message, msg, parse_mode="HTML")
            else:
